...
# created an empty visited dictionaary
visited = {}

# get current room
current_room = player.currentRoom.id

# set directions of current room in visited
visited[current_room] = build_exits()
...
# push a list holding the starting direction
s.push([find_unexplored_direction(exits)])


# while the queue is not empty
while s.size() > 0 and len(visited) < len(roomGraph):
    # pop to the path
    traversalPath = s.pop()
    ...
    # # if directions is not all visited
    # if not is_all_visited(visited[current_room]):
    #     # add direction to visited direction
    #     visited.add(vert)
    ...
    if s.size() == 0 and len(visited) < len(roomGraph):
        ...
        backwards_direction = traversalPath[1:-1]

        d = backwards_direction.pop()

        next_d = find_unexplored_direction(visited[current_room])

        while next_d is None:
            player.travel(d)
